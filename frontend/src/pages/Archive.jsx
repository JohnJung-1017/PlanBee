  ...
  const handleDateSearch = async () => {
    ...
    try {
      const response = await axios.get(
        `https://planbee.p-e.kr/planbee/archive/searchDate/${yymmdd}`,
  ...
  const handleSearch = async () => {
    ...
    try {
      ...
      if (selectedDate) {
        ...
        response = await axios.get(
          `https://planbee.p-e.kr/planbee/archive/searchKeyword/${keyword}`,
          { withCredentials: true }
        );
      }
  ...
  useEffect(() => {
    const initialize = async () => {
      try {
        // 초기 데이터 6개 요청
        const response = await axios.get(
          `https://planbee.p-e.kr/planbee/archive?page=0&limit=6`,
          { withCredentials: true }
        );
  ...
  const handlePrev = async () => {
    ...
    if (cachedArchive.length >= 2) {
      ...
      try {
        ...
        const response = await axios.get(
          `https://planbee.p-e.kr/planbee/archive?page=${nextPage}&limit=6`,
          { withCredentials: true }
        );
        const fetched = response.data;
